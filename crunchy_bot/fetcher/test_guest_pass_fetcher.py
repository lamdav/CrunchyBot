...
@pytest.fixture()
def config() -> Config:
    return Config(crunchy_username="username",
...
def test_fetch(pathlib_mock, options_mock, webdriver_mock, webdriver_wait_mock,
               config, logger):
    ...
    table_mock.find_elements_by_tag_name.return_value = [valid_row_mock, invalid_row_mock]
    valid_guest_pass = "abcd123"
    valid_row = Row(created_at="1/1/2000",
    ...
    invalid_row = Row(created_at="12/30/1999",
                      guest_pass="invalid",
                      description="description",
                      status="Expired",
    ...
    webdriver_wait_mock(driver_mock, 20).until(Any).send_keys.assert_any_call(config.crunchy_username)
    webdriver_wait_mock(driver_mock, 20).until(Any).send_keys.assert_any_call(config.crunchy_password)
    webdriver_wait_mock(driver_mock, 20).until(Any).send_keys.assert_any_call(Keys.ENTER)
    driver_mock.quit.assert_not_called()
    driver_mock.get.assert_any_call("https://www.crunchyroll.com/acct/?action=guestpass")
